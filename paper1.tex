...
As the network evolves, the edges close to sources and drains will be turned on first. A winner-takes-all current path will form, then branches out to the rest of the network. Among all sorts of metrics, voltage distribution on nodes $\vec{V_n}$ and on edges $\vec{V_e}$ would be the most important ones as they determine the network's state.

\paragraph{Voltage distribution} 

Voltage distribution on edges at a specific time $\vec{V_e}(t)$ shows a clear correlation with edge current flow betweenness centrality. Meanwhile, since the filament states $\lambda$ strongly rely on voltage distribution $\vec{V_e}(t)$, a correlation between $\lambda$ and EBC as well. As plotted in fig. \ref{fig:v_lam_cent}, for $\vec{V_e(t)}$ two linear groups 
